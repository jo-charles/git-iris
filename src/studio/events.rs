...
fn handle_review_key(state: &mut StudioState, key: KeyEvent) -> Action {
    use super::state::PanelId;

    match state.focused_panel {
        PanelId::Left => handle_review_files_key(state, key),
        PanelId::Center => handle_review_diff_key(state, key),
        PanelId::Right => handle_review_output_key(state, key),
    }
}

/// Sync file tree selection with diff view in review mode
fn sync_review_file_selection(state: &mut StudioState) {
...
fn handle_review_files_key(state: &mut StudioState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Char('j') | KeyCode::Down => {
            ...
            sync_review_file_selection(state);
            state.mark_dirty();
            Action::Redraw
        }
        KeyCode::Char('k') | KeyCode::Up => {
...
fn handle_review_diff_key(state: &mut StudioState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Char('j') | KeyCode::Down => {
...
fn handle_review_output_key(state: &mut StudioState, key: KeyEvent) -> Action {
    match key.code {
