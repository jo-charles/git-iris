...
impl IrisAgent {
    ...
    async fn gather_intelligent_context(
        ...
        log_debug!(
            "🧠 Iris: Starting intelligent context analysis with agent-driven tool selection"
        );
        ...
        log_debug!("🔧 Iris: Executing agent-planned tool calls");
        let tool_results = self.execute_planned_tool_calls(context, &tool_plan).await?;
        log_debug!("✅ Iris: Completed {} tool operations", tool_results.len());
        ...
        log_debug!(
            "🤖 Iris: LLM synthesis response received - {} chars",
    ...
    async fn plan_tool_usage_for_context_analysis(
        ...
        log_debug!("🤖 Iris: Agent planning tool usage strategy");
        ...
        log_debug!("🔧 Iris: Available tools: {}", available_tools.len());
        ...
        log_debug!("🤖 Iris: Sending tool planning request to LLM");
        let planning_result = self.analyze_with_backend(&planning_prompt).await?;
        log_debug!(
            "📋 Iris: Tool planning response received - {} chars",
            planning_result.len()
        );
        ...
        log_debug!(
            "📋 Iris: Parsed {} planned tool operations",
            tool_plan.len()
        );
    ...
    async fn execute_planned_tool_calls(
        ...
        while !current_plan.is_empty() {
            ...
            match tool.execute(context, &params).await {
                ...
                Err(e) => {
                    log_debug!("❌ Iris: Tool call failed for {}: {}", plan.tool_name, e);
    ...
    fn parse_tool_plan(&self, planning_result: &str) -> Result<Vec<ToolPlan>> {
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(planning_result) {
            if let Some(array) = parsed.as_array() {
                ...
                log_debug!(
                    "✅ Iris: Successfully parsed {} tool operations",
                    tool_plan.len()
                );
                return Ok(tool_plan);
            }
        ...
        log_debug!("⚠️ Iris: Failed to parse tool plan, using fallback");
        Ok(vec![ToolPlan {
    ...
    async fn parse_intelligence_result(
        ...
        log_debug!(
            "🔍 Iris: Parsing LLM analysis result: {}",
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(result) {
                    ...
                    files
                        ...
                        .filter_map(|(i, file)| {
                            ...
                            if let Some(ref fr) = file_result {
                                log_debug!(
                                    "📄 Iris: File {} analysis - relevance: {:.2}, {} key changes",
                                    fr.path,
                                    fr.relevance_score,
                                    fr.key_changes.len()
                                );
            ...
            log_debug!(
                "✅ Iris: Successfully parsed intelligent context with {} file analyses",
            ...
            log_debug!(
                "⚠️ Iris: JSON parsing error - raw response: {}",
            ...
            let files_with_relevance: Vec<FileRelevance> = git_context
                ...
                .map(|(i, file)| {
                    log_debug!(
                        "📄 Iris: Creating fallback analysis for file {}: {}",
                        i + 1,
                        file.path
                    );
                    FileRelevance {
            ...
            log_debug!(
                "⚠️ Iris: Created fallback context with {} files",
                files_with_relevance.len()
            );
    ...
    async fn build_commit_context(
        ...
        let git_context = context.git_repo.get_git_info(&context.config).await?;
        log_debug!(
            "🏗️ Iris: Git context retrieved with {} staged files",
            git_context.staged_files.len()
        );
        ...
        for staged_file in &mut staged_files {
                ...
                .find(|f| f.path == staged_file.path)
            {
                ...
                enhanced_count += 1;
                log_debug!(
                    "🔧 Iris: Enhanced file {} with {} key changes (relevance: {:.2})",
                    staged_file.path,
                ...
                log_debug!(
                    "⚠️ Iris: No intelligent analysis found for file: {}",
                    staged_file.path
                );
            }
        }
        ...
        log_debug!(
            "🏗️ Iris: Enhanced {} of {} files with intelligent analysis",
            enhanced_count,
            staged_files.len()
        );
        ...
        T: for<'de> Deserialize<'de>,
    {
        ...
        if let Some(json_start) = response.find("```json") {
            log_debug!(
                "🔍 Iris: Found markdown JSON block at position {}",
                json_start
            );
            ...
            if let Some(json_end_relative) = response[content_start..].find("```") {
                ...
                log_debug!(
                    "📄 Iris: Extracted JSON content - {} chars",
        ...
        if let Some(start) = response.find('{') {
            if let Some(end) = response.rfind('}') {
                ...
                log_debug!(
                    "🔍 Iris: Found potential JSON object from {} to {} - {} chars",
                    start,
                    end,
                    potential_json.len()
                );
                log_debug!(
                    "📄 Iris: Potential JSON preview: {}",
        ...
        if let Some(start) = response.find('{') {
            ...
            if last_valid_end > start {
                ...
                log_debug!(
                    "🔧 Iris: Attempting to parse truncated JSON - {} chars",
                    truncated_json.len()
                );
                if let Ok(parsed) = serde_json::from_str::<T>(truncated_json) {
                    log_debug!("✅ Iris: Truncated JSON parsing successful");
                    return Ok(parsed);
                }
            }
        }
...
impl super::core::IrisAgent for IrisAgent {
    ...
    async fn execute_task(
        ...
        if !self.initialized {
            log_debug!(
                "❌ Iris: Agent not initialized, cannot execute task: {}",
                task
            );
        ...
        match &result {
            Ok(task_result) => {
                log_debug!(
                    "✅ Iris: Task '{}' completed successfully in {:.2}s (confidence: {:.2})",
                    task,
