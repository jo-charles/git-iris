...
pub struct ReleaseNotesRequest {
    ...
    #[schemars(
        description = "Ending reference (commit hash, tag, or branch name). Defaults to HEAD if empty or not specified."
    )]
    #[serde(default)]
    pub to: String,
    ...
    #[schemars(
        description = "Level of detail for the release notes. Values: 'minimal', 'detailed'. Defaults to 'standard' if empty or not specified."
    )]
    #[serde(default)]
