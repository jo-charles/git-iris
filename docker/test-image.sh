...
if [ -n "$OPENAI_API_KEY" ]; then
  ...
  MODEL="test-model"
  REAL_API_KEY=false
  echo -e "${YELLOW}⚠️ Using test provider (no OPENAI_API_KEY found in environment)${NC}"
  echo -e "${YELLOW}⚠️ Note: Tests with the test provider may fail in production builds${NC}"
  echo -e "${YELLOW}⚠️ Limited tests will be performed without a real API key${NC}"
fi
